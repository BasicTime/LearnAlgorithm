...
知道思路，多练习，总结代码块习惯
## 一些技巧
> 1. mid = (L+R)/2 => mid = L+((R-L)>>1);//计算中数  
> 2. swap(int [] arr,int idx1,int idx2){  
...
实现上具有两个游标，一前一后，用size变量把head、tail变量解耦，更方便控制。

### 18.实现一个特殊的栈，在实现栈的基本功能基础上，再实现返回栈中最小元素操作。  
要求：  
1. pop、push、getMin操作的时间复杂度都是O(1)
2. 设计的栈类型可以使用现成的栈结构

### 19.仅用队列结构实现栈结构？仅用栈结构实现队列结构？
> 队列结构实现栈结构  
...
思路二：
1) 整个拆分为两部分，分为给定两个点，打印之间的一个圈，或之间的一条线、或一个点
2) 控制控制好这两个点的边界

### 22.给定一个arr[N][N]正方形，顺时针旋转90度
思路：
1. 分为给定起始结束共计两个点的坐标围成的圈旋转
2. 跳转到内圈继续旋转打印
